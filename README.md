...
Therefore, since the limits -180° and 180° cannot be exceed, when the robot has to pass from a positive heading (close to 180°) to a negative one (close to -180°), or viceversa, the turning logic implemented by the function `turn_ang()` doesn't make the robot cover the smaller angle because this would mean, at a certain point, to jump from 180° to -180°, or viceversa. Instead, the function makes ithe robot turns in the opposite way in order to decrese (or increase) its current heading gradually until the final orientation is reached. 
