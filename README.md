...
Therefore, since the limits -180° and 180° cannot be exceed, when the robot has to pass from a positive heading (close to 180°) to a negative one (close to -180°), or viceversa, the turning logic implemented by the function `turn_ang()` doesn't make the robot cover the smaller angle (this would mean to jump from 180° to -180° or viceversa) but rather makes it turn in the opposite way to decrese (or increase) its current heading step by step. 
